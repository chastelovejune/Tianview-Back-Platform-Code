...
var artDialog = function (config, ok, cancel) {
    ...
    if (typeof config === 'string' || config.nodeType === 1) {
        ...
        config = {content: config.content, fixed: !_isMobile};
    };
...
artDialog.fn = artDialog.prototype = {
    ...
    constructor: function (config) {
		...
		//dom.reload[config.cancel === false ? 'hide' : 'show']();
    ...
    */
    content: function (message) {
		...
		{
			sel=this;
			$content.load(message,function(rs){/*如果返回内容为json，则处理json数据*/if(rs.indexOf('{"isOk"')!==-1){o=eval("(" + rs + ")");if(o.isOk==0) $content.html(o.msg);} /*初始化窗口大小*/sel._reset()});
			//this._reset();
		}
    ...
    */
    ...
    _addEvent: function () {
        ...
        .bind('click', function (event) {
            ...
            else if (target == "javascript:reload;") {
				//alert('reload');
                that._click('reload');
                return false;
            } else {
...
artDialog.pos=function(id){
	var dialogs = artDialog.list[id];
	//alert(dialogs._reset());
	dialogs._reset();
};
...
+                                       '<a class="d-reload" href="javascript:reload;"></a> <a class="d-close" id="{id}_close" href="javascript:;">×</a>'
+                                   '</div>'
+                               '</td>'
+                           '</tr>'
+                           '<tr>'
